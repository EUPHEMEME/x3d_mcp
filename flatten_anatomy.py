...
each segment; the teeth need their own -- the naming triple the UI drives:
...
# The model has 257 clickable parts but a human skeleton is canonically 206
...
# every part here means the explorer's "why 257, not 206?" panel is grounded in
...
def main() -> None:
    ...
    while changed:
        ...
        for inl in list(root.iter("Inline")):
            ...
            has_geom = any(True for g in GEOM for _ in wrap.iter(g))
            if has_geom and bone not in seen:
                seen.add(bone)
                wrap.set("DEF", bone)
                phong += phong_to_physical(wrap)

                # every material in this bone -> MAT_<bone>, USE links preserved
                renamed: dict[str, str] = {}
                for m in wrap.iter("PhysicalMaterial"):
                    old = m.get("DEF")
                    if old and not m.get("USE"):
                        new = f"MAT_{bone}"
                        m.set("DEF", new)
                        renamed[old] = new
                        break                      # one canonical material per bone
                for m in wrap.iter("PhysicalMaterial"):
                    u = m.get("USE")
                    if u and u in renamed:
                        m.set("USE", renamed[u])
                for m in wrap.iter("PhysicalMaterial"):   # anonymous: name it
                    if not m.get("USE") and not m.get("DEF"):
                        m.set("DEF", f"MAT_{bone}")
                        break

                # sensor as a sibling of this bone's geometry.
                # description='' on purpose: X_ITE renders TouchSensor.description
                # as a native tooltip, which would fight the HTML label overlay.
                ts = etree.Element("TouchSensor")
                ts.set("DEF", f"TS_{bone}")
                ts.set("description", "")
                wrap.insert(0, ts)
                ...
                bones.append({
                    "name": bone,
                    "display": display_name(bone),
                    "region": region_of(bone),
                    "kind": kind_of(bone),
                    "groupFallback": group_of(bone),
    ...
    manifest = {
        ...
        # measured, not asserted: the raw material for the "why 257, not 206?" panel
        "kinds": dict(kinds),
