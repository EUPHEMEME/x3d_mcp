...
\author{Alexander Hoffman\\[2pt]
  \small\textit{Draft --- see the Use of Generative AI statement}}
\date{\today}
...
In keeping with the ACM Policy on Authorship, the generative AI is \emph{not}
credited as an author; it is disclosed as a tool, used under direction (see the
Use of Generative AI statement below). The intent is candor in both directions:
...
involved. The human author bears full accountability for every assertion in the
paper and every artifact in the archive. Speed without accountability is not a
contribution; the point of the pair is that a person remains answerable for what
the fast tool produces.

\paragraph{Use of generative AI.} Anthropic's Claude (Claude Code) was used as a
tool throughout this work --- procedural generation, schema and semantic
validation, the verify-by-render passes, and drafting --- under the human
author's direction. Per the ACM Policy on Authorship, the generative AI is not
credited as an author: the human author conceived the work, supplied all sources
and domain knowledge, made every substantive and aesthetic decision, and is
solely responsible for the content. This statement discloses its use as required.
