...
| 5 | **Rigging** | Skeleton + deformers + controls | **HAnim** quadruped (34 joints, LOA5-spirit) + **`skinCoord`/`skinCoordWeight`** linear-blend skinning of trunk **and** legs; **2-bone foot IK** keeps the hooves planted while the body shifts weight (runtime re-skinning needs the `joints` USE-list) | ✅ |
| 6 | **Animation** | Author the performance | A keyframe **director** applying the animation principles: easing (slow-in/out), **anticipation**, **overlapping action** (the neck chain staggers into an arc), **overshoot/settle**, **secondary action** (ears/tail lag), and **moving holds** — all baked into eased sub-keys. Drives the graze cycle and the researched **dive** (coil → plunge → bottom-browse → surface) | ✅ |
